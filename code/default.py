...
from xgboost import XGBRegressor # YENİ: XGBoost Import
...
print("5: XGBoost Regressor") # YENİ SEÇENEK
secim = input("Lütfen Eğitmek İstediğiniz Modeli seçiniz (1-5): ")
...
if secim == '1':
    ...
    MODEL_TO_TEST = 'XGBoost' # YENİ SEÇİM
else:
    raise ValueError("Geçersiz seçim! Lütfen 1-5 arasında bir değer giriniz.")
#--------------------------------------------------------------------------------
...
if X is not None:
    ...
    elif MODEL_TO_TEST == 'XGBoost': # YENİ MODEL TANIMI
        model = XGBRegressor(random_state=42, n_jobs=-1)
        MODEL_ADI = "XGB_Default"
    else:
    ...
    try:
        if hasattr(model, 'feature_importances_'): 
            ...
            print("Özellik önemi 'coef_' ile hesaplanıyor...")
            importances = np.abs(model.coef_)
            feat_imp = pd.Series(importances, index=feature_names).sort_values(ascending=False)
        else:
            raise AttributeError("Model desteklemiyor.")
